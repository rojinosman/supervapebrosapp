...
export const VapeCard = memo(function VapeCard({ product, expanded, onToggle, onSetFlavorStock, onRemove, onAddFlavor }: Props) {
  ...
  const confirmRemove = () => {
    Alert.alert(
      "Remove product?",
      `This will permanently remove "${product.name}" from the backend (and every device).`,
      [
    ...
    <View style={[styles.card, expanded && styles.cardExpanded]}>
      ...
      {expanded && product.flavors.length > 0 && (
        <View style={styles.expanded}>
          ...
          {/* Remove product */}
          <Pressable onPress={confirmRemove} style={({ pressed }) => [styles.removeBtn, pressed && { opacity: 0.9 }]}>
